...
> Querying ISMA for prose depth? Use `/search` (V1, full `ISMA_Quantum` corpus — or MCP `isma_search`), `top_k≥25`, do a `scale:"full_4096"` pass + a `search_512` pass, multi-phrase the query, expand the docs that matter, NEVER use an HMM/motif/enriched_only path, avoid plain `/v2/search` and `/v2/search/semantic` because they query the partial shadow class, and remember that `/v2/search/adaptive` is supported. Cross-check any number against your fleet's canonical-baselines artifact. Go deep — a handful of tiles is a failed query.
