...
- ❌ NEVER: `/search/hmm`, `/v2/search/hmm`, `/search/motif`, `isma_motif_search`, or `enriched_only=true`. `/search/hmm` scale-collapses and under-covers prose (measured 2026-08-04, PR #50). `/v2/search/hmm`, `/search/motif`, `isma_motif_search`, and `enriched_only=true` were NOT MEASURED in that work; avoid them for prose depth pending measurement.
