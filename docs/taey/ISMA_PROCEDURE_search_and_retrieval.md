...
- **Never plain `/v2/search` or `/v2/search/semantic`.** Those routes query `ISMA_Quantum_v2`, a ~4.6%
  partial migration (73,809 tiles vs 1,605,584) that receives no new writes and contains **zero** tiles
  of recently authored documents. Measured: the same query scored **0.650 on V1 vs 0.072 on v2**. A v2
  answer looks plausible while being built on a fraction of what we know — a near-miss that resembles
...
- **`isma_adaptive_search` and `/v2/search/adaptive` are SUPPORTED** — corrected 2026-07-30. An
  ...
  **V1-based with a V2 overlay** (its implementation uses V1 `hybrid_retrieve_hmm` as the base and
  sets `V1_CLASS = ISMA_Quantum`), measured at **0.650–0.700** — equal to canonical `/search` — and
  it returns documents authored today that have zero tiles in the v2 class. `/search` is still the
  canonical prose route; adaptive is simply not the thing to avoid.
