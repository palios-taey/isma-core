...
recorded as a `CORRECTION` event before the old tile is hidden. **Decay classes:** session/working
...
| Revision / contest state | Explicit transition hooks (`ISMACore.mark_revised` and `ISMACore.mark_contested`) materialize `correction_status` on V1 `ISMA_Quantum` tiles without setting `is_superseded=true`. Before either hook clears `is_superseded` / `superseded_by` / `invalidated_at`, it reads the target tile and refuses fail-loud if the tile is already `is_superseded=true` or `correction_status=corrected`. `mark_revised` keeps the prior visible, demotes it to sandbox/advisory, emits a replayable `BRANCH_CREATED` receipt, and writes a Neo4j `REVISES(new)->(old)` edge when graph ids are available; `mark_contested` keeps both sides visible, emits `CONTRADICTION_DETECTED`, and writes the existing `CONTRADICTS` relation. |
| Supersede-on-write | shared-lineage recency defaults to `mark_revised` and leaves prior tiles visible as advisory; hard supersede requires a `refuter={who, source, when}` whose `who` matches the authenticated event actor, stamps that refuter into `provenance_hash`, emits a `CORRECTION` event, and only then sets `is_superseded=true`. Missing, incomplete, or actor-mismatched refuter data raises before mutation; lookup/patch errors remain **fail-loud / fail-closed**. |
